...
class EntityTypes(object):
    def __init__(self, types_path: str, negative_types_number: int, negative_mode: str, context_proto:bool, mask_proto:bool):
        ...
        self.types_mask_embedding = None # mask句子中其余实体的embedding
...
class Corpus(object):
    ...
    def read_tasks_from_file(   #Corpus：（read_tasks_from_file）return4个键值的task变量
        self,
        data_fn,
        update_transition_matrix=False,
        concat_types: str = "None",
    ...
    def _convert_example_to_feature_(
        ...
        #         context_tokens[i]="[MASK]"
        # context_ids=self.tokenizer.convert_tokens_to_ids(context_tokens)  #context的ids
        ...
        if pad_on_left:
            ...
            # context_ids += [pad_token] * padding_length #context 
        ...
        return (
            InputFeatures(
                input_ids=input_ids, #token ids padding至128
                input_mask=input_mask, #token ids为1，cls，sep为-100，padding为-1
                segment_ids=segment_ids, #segment为0，分句子会有1
                label_ids=label_ids,  # 1为B，2为O, 3为E
    ...
    def get_batch_meta(self, batch_size, device="cuda", shuffle=True):   #构造数据
        ...
        for i in range(start_id, start_id + batch_size):
            ...
            query_item = {
                ...
                #     [f.context_ids for f in task_curr["query"]], dtype=torch.long
                # ).to(
                #     device
                # ),  # context ids
            ...
            support_item = {
                ...
                # "context_ids": torch.tensor(
                #     [f.context_ids for f in task_curr["support"]], dtype=torch.long
                # ).to(device),
                # # 1 x max_seq_len
