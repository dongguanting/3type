...
class FGM():
    ...
    def attack(self, epsilon, emb_name='embeddings.LayerNorm.weight'):
...
class Learner(nn.Module):
    ...
    def inner_update(self, data_support, lr_curr, inner_steps, no_grad: bool = False):
        ...
        for i in range(inner_steps):
            ...
            inner_opt.zero_grad()
            _, _, loss, type_loss = self.model.forward_wuqh(  #上下文loss融入typeloss
                input_ids=data_support["input_ids"],
                # context_ids=data_support["context_ids"],
            ...
            # elif type_loss is not None:
            #     loss = loss + type_loss
            # elif type_loss_context is not None:
            #     loss = loss + type_loss_context
            # if no_grad:
            #     continue
            # loss.backward()

            '''在原型网络阶段加入对抗'''
            fgm=FGM(self.model) ##初始化对抗模型
            if self.args.adv_proto:
                
                fgm.attack(1)
                _, _, loss_adv, type_loss_adv = self.model.forward_wuqh(
                input_ids=data_support["input_ids"],
                attention_mask=data_support["input_mask"],
                token_type_ids=data_support["segment_ids"],
                labels=data_support["label_ids"],
                ...
                if loss_adv is None:
                    loss_adv = type_loss_adv
                elif type_loss_adv is not None:
    ...
    def forward_supervise(self, batch_query, batch_support, progress, inner_steps): #没有原型网络的
    ...
    def forward_meta(self, batch_query, batch_support, progress, inner_steps):  #常规走这个
        ...
        for task_id in range(task_num):
            ...
            # if type_loss is not None:
            #     type_losses.append(type_loss.item())
            # if type_loss_context is not None:
            #     type_losses.append(type_loss.item())
            # if loss is None:
            #     loss = type_loss+type_loss_context
            # elif type_loss is not None:
            #     loss = loss + type_loss + type_loss_context
            # grad = torch.autograd.grad(loss, params)
            # meta_grad.append(grad)
            ...
            if self.args.adv_2:
                ...
                if loss_adv is not None:
                    span_losses.append(loss_adv.item())
                if type_loss_adv is not None:
                    type_losses.append(type_loss_adv.item())
                if loss_adv is None:
                    loss_adv = type_loss_adv
                elif type_loss_adv is not None:
                    loss_adv = loss_adv + type_loss_adv
    ...
    def evaluate_meta_(    #只有test走这个
        ...
        if self.eval_mode == "two-stage":  #两阶段，先走span预测
        ...
        if self.eval_mode == "two-stage":   #二阶段走type
    ...
    def load_model(self, mode: str = "all"):
        ...
        self.model.set_config(
            ...
            mode,
            self.args.context_proto,
            self.args.mask_proto,
